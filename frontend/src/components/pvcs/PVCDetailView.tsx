...
interface PVCDetailViewProps {
  namespace: string;
  name: string;
}

export function PVCDetailView({ namespace, name }: PVCDetailViewProps) {
  const { goBack } = useNavigation();
  const [pvc, setPVC] = useState<kube.PVCDetail | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      setError("");
      try {
        const [pvcDetail, pvcEvents] = await Promise.all([
          GetPVCDetail(namespace, name),
    ...
    const cancel = EventsOn(
      ...
      (event: {
        ...
        if (event.type === "DELETED") {
          setError("This persistent volume claim has been deleted");
          setPVC(null);
          return;
        }
