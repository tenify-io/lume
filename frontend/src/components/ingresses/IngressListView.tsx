...
export function IngressListView() {
  ...
  const { items: ingresses, loading } = useResourceList<kube.IngressInfo>({
    fetchItems: () => GetIngresses(selectedNamespace),
  ...
  const filteredIngresses = ingresses.filter(
    (ing) =>
  ...
  return (
    <>
      ...
      {/* Empty state */}
      {!loading && filteredIngresses.length === 0 && (
          ...
          <tbody>
            {filteredIngresses.map((ing) => (
              ...
              >
                ...
                <td className="px-3 py-2 border-b border-zinc-800/30 whitespace-nowrap overflow-hidden text-ellipsis max-w-[300px]">
                  {ing.hosts || "—"}
                </td>
      ...
      <div className="px-6 py-1.5 text-xs text-zinc-600 bg-[#111113] border-t border-zinc-800/50 shrink-0">
        {loading ? "Loading..." : `${filteredIngresses.length} ingress(es)`}
